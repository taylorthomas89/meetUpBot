...
const {returnEvent, isThereUpcomingEvents, _upcomingEvents} = require('../server/utils/eventUtils.js');
...
MeetupBot.prototype._onMessage = function(message) {
  if (this._isChatMessage(message) && this._isChannelConversation(message) && !this._isFromMeetupBot(message) && this._isMentioningMeetupBot(message)) {
...
MeetupBot.prototype._replyWithMeetUps = function(originalMessage) {
  ...
  //Checks if there are meetups and replies with appropriate message
  if (isThereUpcomingEvents()) {
    ...
    self.postMessageToChannel(channel.name, 'There are no upcoming meetups this week.', {as_user: true});
  }
};
...
MeetupBot.prototype._welcomeMessage = function() {
  this.postMessageToChannel(this.channels[0].name, 'Hello World! I can let you know about upcoming meetups in your area. ' + 'Just say `!meetups` to invoke me!', {as_user: true});
};
...
MeetupBot.prototype._isMentioningMeetupBot = function(message) {
  return message.text.toLowerCase().indexOf('!meetup') > -1 || message.text.toLowerCase().indexOf(this.name) > -1 || message.text.toLowerCase().indexOf('!meetups') > -1;
};
