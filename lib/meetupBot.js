...
var {printEvents} = require('./../server.js');
...
MeetupBot.prototype._replyWithMeetUps = function (originalMessage) {
  ...
  self.postMessageToChannel(channel.name, 'Here are your meetups: ', {as_user: true}); // meetUpsGoHere ?? ??? ??
  //self.postMessageToChannel(channel.name, printEvents, {as_user: true});
};

MeetupBot.prototype._loadBotUser = function () {
  console.log('Loading NorrisBot..');
    var self = this;
...
//        return user.name === self.name;
  ...
  console.log(this.user); // Why is this returning undefined?
};
